...
We'll plot viability responses based on each of the individual factors.

```{r}
ggplot(via_dat, aes(x=Cactus, y=viability, colour=Cactus)) + 
  geom_boxplot() +
  xlab("Viability") +
  ylab("Host cactus")
```
...
```{r}
ggplot(via_dat, aes(x=Population, y=viability, colour=Population)) + 
  geom_boxplot() +
...
```{r}
ggplot(via_dat, aes(x=sex_bias, y=viability)) + 
