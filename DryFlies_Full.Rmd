...
## Loading the data
```{r}
decision_dat<-read.csv("./data/CHC_dat.csv")
```
...
```{r}
decision_dat <- decision_dat[1:37]
names(decision_dat)
```
...
```{r}
str(decision_dat) #everything here looks good
```
...
```{r}
Rows<-c(1:nrow(decision_dat))
Train<-Rows %% 2==1
Validate<-Rows %% 2==0

dat_train<-decision_dat[Train,]
dat_validate<-decision_dat[Validate,]
